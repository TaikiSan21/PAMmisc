...
prepTzFix <- function(x, offset=NULL) {
  wavList <- list.files(x, full.names=FALSE, recursive = FALSE, pattern='\\.wav$')
  tAdj <- data.frame(oldName=wavList)
  if(!is.na(processStWavNames(wavList[1], type = 'st'))) {
    type <- 'st'
  } else if(!is.na(processStWavNames(wavList[1], type = 'sm3m'))) {
    type <- 'sm3m'
  } else {
    stop('Could not parse wav file names for time information, send Taiki a file')
  }
  tAdj$oldTime <- processStWavNames(tAdj$oldName, type=type)
  if(is.null(offset)) {
  ...
  for(i in 1:nrow(tAdj)) {
    ...
    newTime <- processStWavNames(tAdj$newTime[i], type=type)
    oldPrefix <-  processStWavNames(tAdj$oldName[i], prefix=TRUE, type=type)
    tAdj$newName[i] <- paste0(oldPrefix, newTime, '.wav')
  }
  if(any(naOff)) {
...
processStWavNames <- function(x, type=c('st', 'sm3m'), prefix=FALSE) {
  type <- match.arg(type)
  switch(type,
         'st' = {
           format <- '%y%m%d%H%M%S'
...
fixStTz <- function(dir, prep, reverse=FALSE, logname='STRenameLog.csv') {
  if(isTRUE(reverse)) {
    ...
    return(fixStTz(dir,revPrep, FALSE, logname='STReverseLog.csv'))
  }
  wavList <- list.files(dir, pattern='wav$', recursive=FALSE)
  ...
  on.exit(write.csv(prep, file=file.path(dir, logname), row.names = FALSE))
